...
class GiroTransporteChasis(DBConnection):
    ...
    def tomarDatosChasisSybase(self):
        ...
        cursor.execute( "select tte_codigo, camion_codigo,chasis_patente, chasis_provincia, acoplado_patente, acoplado_provincia,chofer,tipo_doc, cuit, seguro, seg_chasis_vto from v_giro_transportes where tte_codigo in(1, 15)")
        ...
        if len(items) > 0:
            ...
            self.persistirChasisEnGiro(self.datosParaGiro)
    ...
    def persistirChasisEnGiro(self, datosParaGiro):
        ...
        for chasisPatente, seguroPoliza, tara, vencCCar, seguroPolizaVence, detalle, dnt in datosParaGiro:
            ...
            try:
                ...
                if respChasis.CodigoError == 0:
                    print(":: Modulo Transporte Chasis, patente "+str(chasisPatente)+" se informo con éxito:: ")
                    ...
                    '''sql = ("INSERT INTO giro_chasis (id_giro, patente, seguro, tara, vencCar, vencSeg, detalle, dnt, informado_sn) "
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'S')" )
                    cursor.execute(sql, (idGiro, chasisPatente, seguroPoliza, tara, vencCCar, seguroPolizaVence, detalle, dnt))
                    self.conn.commit()'''
